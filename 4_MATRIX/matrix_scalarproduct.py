...
class MRScaler_Product(MRJob):
    ...
    def steps(self):
        return [MRStep(mapper_raw=self.map_values,      # Mapping the matrix elements to be able to process better
                       reducer=self.reducer_part1),        # Multiplying and tagging elements to sum in the next step
                MRStep(mapper=self.mapper_part2,          # Mapping the tags -> key
                       reducer=self.reducer_part2)]     # Summing the elements with same key, then it is done!
